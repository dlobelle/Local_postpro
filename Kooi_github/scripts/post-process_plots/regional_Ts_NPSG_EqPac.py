...
'''Figure 4 and 5 in JGR:Oceans manuscript: Ts for regional analyses (North Pacific Subtropical Gyre and Equatorial Pacific)'''
''' Only size 1 micron and 0.1 micron used for draft (since required most explanations)'''
...
'''CHOOSE intial density and size, but for manuscript, only rho = 920 and s = 6 and s = 7'''
rho =  '920' # [kgm-3]: density of the plastic #920 #840 #30
s = '6' # 6 or 7: for 10-6 and 10-7 m 
...
proc = ['bfadv','nobf','noadv']
region = ['GPGP','EqPac'] # GPGP is NPSG in manuscript
...
for row in range(2): 
    for col in range(3):
        ...
        ax.title.set_text(f'({letter}) {r_ttl}: {proc_ttl}')
